...
BEGIN
    ...
    IF checker = 'TRUE' THEN
        raise_application_error(-20602,:NEW.bonos_ID || ' No se puede a�adir consumibles a un bono que esta activo');
    END IF;
END;
/
...
BEGIN
    SELECT disponible INTO checker FROM bonos WHERE bonos_ID=:NEW.bonos_ID;
    IF checker = 'TRUE' THEN
        raise_application_error(-20603,:NEW.bonos_ID || ' No se puede a�adir pases a un bono que esta activo');
    END IF;
END;
/ 
